...
EXPORT unsigned char *dwg_bmp (const Dwg_Data *, BITCODE_RL *);
 ...
 * is case-insensitive */
EXPORT BITCODE_H dwg_find_tablehandle (Dwg_Data * dwg,
                                       const char * name,
                                       const char * table);
 ...
 */
EXPORT BITCODE_H dwg_find_table_control (Dwg_Data * dwg,
                                         const char * table);

/** Searching for a dictionary ref.
 *  Returning a hardpointer ref (5) to it, as stored in header_vars. */
EXPORT BITCODE_H dwg_find_dictionary (Dwg_Data * dwg,
                                      const char * name);
/** Searching for a named dictionary entry in the given dict. Search is
 * case-insensitive */
EXPORT BITCODE_H dwg_find_dicthandle (Dwg_Data * dwg, BITCODE_H dict,
                                      const char * name);
/* Search for a table EXTNAME */
EXPORT char *dwg_find_table_extname (Dwg_Data * dwg,
                                     Dwg_Object * obj);
...
EXPORT Dwg_Object *dwg_ref_object (const Dwg_Data * dwg,
                                   Dwg_Object_Ref * ref);

EXPORT Dwg_Object *dwg_ref_object_relative (const Dwg_Data * dwg,
                                            Dwg_Object_Ref * ref,
                                            const Dwg_Object * obj);
EXPORT Dwg_Object *dwg_ref_object_silent (const Dwg_Data * dwg,
                                          Dwg_Object_Ref * ref);

EXPORT Dwg_Object *get_first_owned_entity (const Dwg_Object * hdr);
EXPORT Dwg_Object *get_next_owned_entity (const Dwg_Object * hdr,
                                          const Dwg_Object * current);
EXPORT Dwg_Object *
get_first_owned_subentity (const Dwg_Object * owner);
EXPORT Dwg_Object *
get_next_owned_subentity (const Dwg_Object * owner,
                          const Dwg_Object * current);
EXPORT Dwg_Object *get_first_owned_block (const Dwg_Object *hdr);
EXPORT Dwg_Object *get_last_owned_block (const Dwg_Object *hdr);
EXPORT Dwg_Object *get_next_owned_block (const Dwg_Object * hdr,
                                         const Dwg_Object * current);
...
EXPORT int dwg_resolve_handleref (Dwg_Object_Ref * ref,
                                  const Dwg_Object * obj);
 ...
 */
EXPORT int
dwg_add_handle (Dwg_Handle * hdl, BITCODE_RC code, unsigned long value,
                Dwg_Object * obj);
...
dwg_add_handleref (Dwg_Data * dwg, BITCODE_RC code, unsigned long value,
                   Dwg_Object * obj);
